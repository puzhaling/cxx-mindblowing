...
SearchTable::SearchTable(int x, int y, int w, int h, const char* title) 
  ...
  col_width(8, kColumnWidth);  // steps
}

void SearchTable::PopulateWith(const std::vector<std::pair<RBTreeNode, int>>& nodes) {
  for (const auto& node_info_pair : nodes) {
    data_.push_back(node_info_pair);
  }
}

void SearchTable::draw_cell(TableContext context, int r, int c, int x, int y, 
                            int w, int h) {
  static char s[40];
  switch (context) {
  ...
  case CONTEXT_COL_HEADER:
    ...
    case 7: 
      ...
      break;
    case 8: 
      snprintf(s, sizeof(s), "Шаги поиска"); 
      break;
    }
    fl_push_clip(x, y, w, h);
    fl_draw_box(FL_THIN_UP_BOX, x, y, w, h, color());
  ...
  case CONTEXT_CELL:
    fl_push_clip(x, y, w, h);
    {
      ...
      fl_color(FL_BLACK);
      const auto& node = data_[r].first;    
      int steps = data_[r].second;
      switch (c) {
      case 0: 
